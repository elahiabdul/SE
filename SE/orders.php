...
$db = getDbInstance();
$select = array('id', 'transfarmer','created_by', 'created_at', 'updated_at');

//Start building query according to input parameters.
// If search string
if ($search_string) {
	$db->where('transfarmer', '%' . $search_string . '%', 'like');
    ...
    <div class="row">
        ...
        <div class="col-lg-6">
            <div class="page-action-links text-right">
                <a href="add_order.php?operation=create" class="btn btn-success"><i class="glyphicon glyphicon-plus"></i> Add new</a>
            </div>
        </div>
    </div>
    ...
    <table class="table table-striped table-bordered table-condensed">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><?php 
                   if($row['transfarmer']!='0' &&  $row['transfarmer']!='') {
                    $db->where('tf_id', $row['transfarmer']);
                    $transfers = $db->getOne("transfers");
                   echo  xss_clean($transfers['Bill_of_Material']);
                   }
                   else
                    {
                        echo 'N/A';
                ...
                <td><?php 
                   ...
                   echo  xss_clean($user['user_name']);
                     ?></td>
                
                <td>
                    <a href="view_order.php?id=<?php echo $row['id']; ?>&operation=edit" class="btn btn-primary"><i class="glyphicon glyphicon-edit"></i></a>
